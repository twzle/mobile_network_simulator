...
class UserGenerator {
    public:
        ...
        static std::discrete_distribution<> get_user_id_distribution(); // Получение текущего распределения
        static void set_user_id_distribution(std::discrete_distribution<> new_distrib); // Изменение текущего распределения
    ...
    private:
        ...
        static std::discrete_distribution<> user_id_distrib; // Целочисленное равномерное распределение для нахождения id подключенного пользователя
