...
Channel::Channel()
    : carrier_frequency(0), constant_offset(0)
{
}

Channel::Channel(double carrier_frequency, int constant_offset)
    : carrier_frequency(carrier_frequency), constant_offset(constant_offset) {}

/*
PathLoss по модели COST231
*/
double Channel::get_path_loss(double user_distance, double bs_height, double user_height)
{
    double log_freq = std::log10(carrier_frequency);
    double log_height_bs = std::log10(bs_height);
    double log_distance = std::log10(user_distance);
...
double Channel::get_interference_power(double user_received_signal_power)
{
    ...
    return user_received_signal_power + bs_to_ue_interference_offset;
